...
parser wiki2beamer:
	...
	rule latex_w2b:
		{{ result = [] }}
		( latex_w2b_entity {{ result.append(latex_entity) }} )*
		{{ return ('LATEX_W2B', result) }}
	...
	rule latex_w2b_entity:
		{{result = []}}
		(
			latex_entity	{{result.append(latex_entity)}}
		|	w2b_listblock	{{result.append(w2b_listblock)}}
		)*
		{{return result}}
	...
	rule w2b_listblock:
		{{ result = [] }}
		(
			( W2B_STAR {{result.append(W2B_STAR)}} | W2B_HASH {{result.append(W2B_HASH)}} )
			( W2B_STAR {{result.append(W2B_STAR)}} | W2B_HASH {{result.append(W2B_HASH)}} )*
			[latex_param_angle_brackets {{result.append(latex_param_angle_brackets)}}]
			SPACE
			w2b_single_line
			NEWLINE
			{{result.extend([SPACE,w2b_single_line,NEWLINE])}}
		)*
		{{return result}}
